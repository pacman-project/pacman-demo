...
public:
    ...
    GraspBackup(ros::NodeHandle nh) : nh_(nh), priv_nh_("~")
    {
      
      srv_grasp_planner_ = nh_.advertiseService(nh_.resolveName("/grasp_planner_srv"),&GraspBackup::extractGrasp, this);
      //path_to_dir = "/home/pacman/Documents/backup-grasps-models-multi-grasps_left/";
//       if( arm_ == "right" )
//         path_to_dir = "/home/pacman/Documents/backup-grasps-models-multi-grasps_right/";
//       else if(arm_ == "left")
//         path_to_dir = "/home/pacman/Documents/backup-grasps-models-multi-grasps_left/";
...
void GraspBackup::set_arm(string arm)
{
  if( arm == "right" )
   // path_to_dir = "/home/pacman/Documents/backup-grasps-models-multi-grasps_right/";
...
void GraspBackup::visualize_gripper(geometry_msgs::PoseStamped gripper_pre_pose,geometry_msgs::PoseStamped gripper_pose)
{
...
std::vector<double> GraspBackup::euler_to_quaternion(vector<double> vals)
{
...
geometry_msgs::PoseStamped GraspBackup::find_transformation(geometry_msgs::Pose ref_obj,geometry_msgs::Pose cur_obj,geometry_msgs::PoseStamped ref_grasp)
{  
...
void GraspBackup::poseEigenToMsg(const Eigen::Affine3d &e, geometry_msgs::PoseStamped &m)
{
...
vector<string> GraspBackup::giveAllFiles(string obj_id)
{
...
geometry_msgs::Pose GraspBackup::searchPoseFile(string obj_id)
{
...
vector<geometry_msgs::PoseStamped> GraspBackup::searchGraspFile(vector<string> path_to_obj_dir,bool pre_grasp)
{
...
bool GraspBackup::extractGrasp(definitions::GraspPlanning::Request  &req, definitions::GraspPlanning::Response &res)
{ 
  grasp_score_.clear();
  bool success = false;
  ...
  for( size_t i = 0; i < grasps.size(); i++ )
  {
    ...
    /*  old_traj.grasp_trajectory[0].wrist_pose.pose = pre_grasps[i].pose;
      old_traj.grasp_trajectory[0].wrist_pose.pose.orientation = old_pre_grasp.pose.orientation;
      old_traj.grasp_trajectory[1].wrist_pose.pose = grasps[i].pose;
      old_traj.grasp_trajectory[1].wrist_pose.pose.orientation = old_grasp.pose.orientation;
      old_traj.grasp_trajectory[2].wrist_pose.pose = grasps[i].pose;   
      old_traj.grasp_trajectory[2].wrist_pose.pose.orientation = old_grasp.pose.orientation;*/
...
void GraspBackup::evaluate_grasp(vector<geometry_msgs::PoseStamped> pre_grasps,vector<geometry_msgs::PoseStamped> grasps,string obj_name)
{
...
{
  ros::init(argc, argv, "graspBackup");
  ros::NodeHandle nh;
  ros::Rate loop_rate(15);
  ros::Timer delay();
 
  GraspBackup grasp(nh);
  ROS_INFO("Grasp Backup node ready to be called...");
  while(ros::ok())
  {
    ros::Rate r(30);
    r.sleep();
