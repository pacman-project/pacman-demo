...
namespace pacman {
    ...
    void ActiveSensOnlineModel::setTrainingData(grasp::data::Item::Map::iterator contactModel){

        //trainingData.clear();

		// collect data (TODO: Receive a list of ItemContactModel
        const grasp::data::ItemContactModel* model = grasp::is<const grasp::data::ItemContactModel>(contactModel);
		if (model)
		{
	...
	void ActiveSensOnlineModel::updateViewingAngles(const HypothesisSensor::Seq& visitedHypotheses){
		...
		for (HypothesisSensor::Seq::const_iterator it_h = visitedHypotheses.begin(); it_h != visitedHypotheses.end(); it_h++)
		{
			...
			for (grasp::Contact3D::Seq::const_iterator it = graspContacts.begin(); it != graspContacts.end(); it++, i++)
			{
                ...
                (*it_h)->getFrame().R.getColumn(2, v); //viewDir of hypothesis sensor

				golem::Vec3 n(0.0, 0.0, 0.0);
                frame.R.getColumn(2,n);//(3, n); //Surface Normal vector of contact point
				n.normalise();
				v.normalise();
	...
	*/
	golem::Real ActiveSensOnlineModel::computeValue(HypothesisSensor::Ptr hypothesis, const grasp::Contact3D::Seq& graspContacts, int index){
		...
		for (grasp::Contact3D::Seq::const_iterator it = graspContacts.begin(); it != graspContacts.end(); it++, i++)
		{
			...
			golem::Vec3 v(0.0, 0.0, 0.0);
            hypothesis->getFrame().R.getColumn(2, v); //viewDir of hypothesis sensor
