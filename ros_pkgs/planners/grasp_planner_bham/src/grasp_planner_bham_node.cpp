...
bool GraspPlanner::planGrasp(definitions::GraspPlanning::Request  &req, definitions::GraspPlanning::Response &res)
{
  ...
  std::string path_to_database("/home/crosales/Code/pacman/pacman/resources/Bham/");
  ...
  ROS_INFO("planned_grasps_.size() %ld",planned_grasps_.size());
  ROS_INFO("planned_grasps_[0].trajectory.size() %ld", planned_grasps_[0].trajectory.size());
  ROS_INFO("planned_grasps_[0].trajectory[0].pose.p.x %f", planned_grasps_[0].trajectory[0].pose.p.x);
  ...
  // temp var to fill the eximation adn convert to quaternion for the orientation part of the pose
  ...
  for (int t = 0; t <  planned_grasps_.size(); t++ )
  {
    ...
    {
      wrist_pose(0,0) = planned_grasps_[t].trajectory[w].pose.R.m11;
      wrist_pose(0,1) = planned_grasps_[t].trajectory[w].pose.R.m12;
      wrist_pose(0,2) = planned_grasps_[t].trajectory[w].pose.R.m13;
      wrist_pose(0,3) = planned_grasps_[t].trajectory[w].pose.p.x;
      wrist_pose(1,0) = planned_grasps_[t].trajectory[w].pose.R.m21;
      wrist_pose(1,1) = planned_grasps_[t].trajectory[w].pose.R.m22;
      wrist_pose(1,2) = planned_grasps_[t].trajectory[w].pose.R.m23;
      wrist_pose(1,3) = planned_grasps_[t].trajectory[w].pose.p.y;
      wrist_pose(2,0) = planned_grasps_[t].trajectory[w].pose.R.m31;
      wrist_pose(2,1) = planned_grasps_[t].trajectory[w].pose.R.m32;
      wrist_pose(2,2) = planned_grasps_[t].trajectory[w].pose.R.m33;
      wrist_pose(2,3) = planned_grasps_[t].trajectory[w].pose.p.z;
      wrist_pose(3,0) = 0.0;
      wrist_pose(3,1) = 0.0;
      wrist_pose(3,2) = 0.0;
      wrist_pose(3,3) = 1.0;

      tf::poseEigenToMsg(wrist_pose, pose.pose);

      //ROS_INFO("pose.pose.position.x %f", pose.pose.position.x);
      
      estimated_grasps[t].grasp_trajectory[w].wrist_pose.pose = pose.pose;
    }
  }
